import getAllTimelines from './../getAllTimelines.service';
...
test('should return a timelines object with paginated data', async () => {
    const page = 1
    const limit = 2

    jest.spyOn(repository, 'getAllRep').mockResolvedValue(mock as any)

    const result = await getAllTimelines(page, limit, repository);
